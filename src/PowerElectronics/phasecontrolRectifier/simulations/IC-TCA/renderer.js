...
function generateChartData(sliderVal) {
  ...
  chart3data = Array.from({ length: numPoints }, (_, i) => {
    ...
    if (isOddCycle &&  ((positionInCycle % (Math.PI/3)) / (Math.PI/3) ) > Math.max(0, sliderVal * 0.01)) {
        return 1;
    } else {
        return 0;
    }
  ...
  chart4data = Array.from({ length: numPoints }, (_, i) => {
    ...
    if (isEvenCycle && ((positionInCycle % (Math.PI/3)) / (Math.PI/3) ) > Math.max(0, sliderVal * 0.01)) {
...
chart6data = Array.from({ length: numPoints }, (_, i) => {
  const xValue = (i * (piCycle * Math.PI) / (numPoints )) - (sliderVal * 0.01 * Math.PI);
    const positionInCycle =  (xValue % Math.PI)-3;
    const isOddCycle = Math.floor(xValue / Math.PI) % 2 === 0;
    if (isOddCycle && positionInCycle > 0 ) {
      return 0.5 - (positionInCycle / (Math.PI * 0.5));
